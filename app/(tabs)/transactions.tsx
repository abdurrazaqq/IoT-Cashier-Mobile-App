...
export default function TransactionsScreen() {
  ...
  const initializeScreen = async () => {
    try {
      ...
      // 🔹 Auto delete transaksi lebih dari 7 hari
      const deleted = await DatabaseService.deleteOldTransactions(7);
      if (deleted > 0) {
        console.log(`${deleted} transaksi lama dihapus otomatis`);
      }

      await loadTransactions(role === 'cashier' ? user?.id : undefined);
    } catch (error) {
  ...
  const loadTransactions = async (cashierId?: string) => {
    try {
      setLoading(true);
      const transactionList = await DatabaseService.getTransactions(cashierId);
  ...
  const onRefresh = () => {
    setRefreshing(true);
    loadTransactions(userRole === 'cashier' ? currentUserId || undefined : undefined);
  };
  ...
  const getPaymentMethodColor = (method: string) => {
    switch (method) {
      ...
      case 'digital': return '#7C3AED';
      default: return '#64748B';
    }
  };

  const getTotalAmount = () => transactions.reduce((sum, t) => sum + t.total, 0);
      ...
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.subtitle}>
            {transactions.length} transactions • Total: Rp {getTotalAmount().toLocaleString('id-ID')}
          </Text>
        </View>
        <TouchableOpacity 
            ...
            <View style={styles.transactionHeader}>
              ...
              <View style={styles.transactionAmount}>
                ...
                <View style={[
                  ...
                  { backgroundColor: getPaymentMethodColor(transaction.paymentMethod) }
                ]}>
...
const styles = StyleSheet.create({
  ...
  deleteButton: {
    ...
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: '#EF4444', // merah untuk tombol hapus
    paddingVertical: 8,
    paddingHorizontal: 12,
    borderRadius: 12,
    marginTop: 12,
    alignSelf: 'flex-end', // agar tombol berada di kanan
  },
  deleteButtonText: {
    color: '#FFFFFF',
    fontWeight: '600',
